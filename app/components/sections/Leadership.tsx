...
const CARD_ICONS: Record<string, React.ReactNode> = {
  '01': <path d="M2 12h20M12 2a15 15 0 010 20M12 2a15 15 0 000 20" />,
  '02': (
    <g>
  ...
  '03': <path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z" />,
  '04': <path d="M17 3a2.828 2.828 0 114 4L7.5 20.5 2 22l1.5-5.5L17 3z" />,
  '05': <path d="M21 15a2 2 0 01-2 2H7l-4 4V5a2 2 0 012-2h14a2 2 0 012 2z" />,
  '06': <path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z" />,
};

function LocalLeadershipCard({ num, title, org, desc, index, dict }: { num: string; title: string; org: string; desc: string; index: number; dict: any }) {
  return (
    <div className="group relative flex flex-col justify-between bg-white px-6 py-8 sm:p-8 lg:p-10 transition-all duration-500 ease-out overflow-hidden hover:shadow-[0_20px_40px_rgba(15,23,42,0.03)]">
      ...
      <div className="relative z-10 mt-8 flex items-center gap-1.5 text-xs font-medium text-blue-600 transition-all duration-300 opacity-100 translate-x-0 md:opacity-0 md:-translate-x-2 md:group-hover:opacity-100 md:group-hover:translate-x-0">
        <span>{dict.leadership.viewInitiatives}</span>
    ...
    <section className="relative bg-white py-20 sm:py-28 md:py-32 lg:py-36 border-b border-slate-100 overflow-hidden" id="leadership">
      <Container>
        ...
        <div className="mb-12 sm:mb-16 md:mb-20 lg:mb-24 text-slate-900">
          <SectionHeader
            label={dict.leadership.label}
            title={
              <>
                {dict.leadership.title}
                <br />
                <em className="font-serif italic text-blue-600 font-normal">{dict.leadership.titleEm}</em>
              </>
            }
          />
        </div>
        ...
        <div className="grid grid-cols-1 gap-px bg-slate-100 border-y border-slate-100 sm:grid-cols-2 md:grid-cols-3">
          {dict.leadership.cards.map((card: any, index: number) => (
            <LocalLeadershipCard key={index} {...card} index={index} dict={dict} />
          ))}
        </div>
